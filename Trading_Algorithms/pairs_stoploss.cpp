...
int main(int argc, char* argv[]) {
    ...
    // double stop_loss_threshold=0.5;
    ...
    if(argc!=10){
        ...
        stop_loss_threshold=stod(argv[7]);
    }
    int extra_days=n-1;
    ...
    for (int i=n; i<Data.size(); i++) {
        if(Data[i].getz_score() > threshold+stop_loss_threshold && holding>0){
            Data[i].buy();
            holding++;
            Data[i].setQuantity(holding);
        }
        else if(Data[i].getz_score() < -threshold-stop_loss_threshold && holding>0){
            Data[i].sell();
            holding--;
            Data[i].setQuantity(holding);
        }
    ...
    //     cout << "Date: " << entry.getDate() << ", Z-score: " << entry.getz_score() << ", Action: " << entry.getAction() << ", Quantity: " << entry.getQuantity() << endl;
    // }
